...
interface MockDisplayLogic {
    operator: 'AND' | 'OR';
    conditions: MockCondition[];
    logicSets?: MockLogicSet[];
}

interface MockSkipLogicSimple {
    type: 'simple';
    isConfirmed: boolean;
    /** 'next' | 'end' | 'block:<blockId>' | question id */
    skipTo: string;
}

interface MockSkipLogicPerChoice {
    type: 'per_choice';
    rules: Array<{ choiceId: string; skipTo: string; isConfirmed: boolean }>;
}

type MockSkipLogic = MockSkipLogicSimple | MockSkipLogicPerChoice;
...
interface MockBranchingCondition {
    id: string;
    questionId: string;
    operator: ConditionOperator;
    value?: string;
...
interface MockBranchingBranch {
    id: string;
    pathName?: string;
    operator: 'AND' | 'OR';
    conditions: MockBranchingCondition[];
    thenSkipTo: string;
    thenSkipToIsConfirmed: boolean;
}

interface MockBranchingLogic {
    branches: MockBranchingBranch[];
...
function fmt(cond: MockCondition | MockBranchingCondition): string {
...
function fmtDest(dest: string, survey: MockSurvey): string {
    ...
    if (dest.startsWith('block:')) {
        ...
        return block ? `Block ${block.bid}: ${block.title.slice(0, 30)}` : 'Unknown Block';
    }
    const q = survey.blocks.flatMap((b) => b.questions).find((q) => q.id === dest);
...
function DisplayLogicDisplayMock({
    ...
    if (confirmedConditions.length === 0 && confirmedSets.length === 0) return null;
        ...
        >
            ...
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-semibold text-foreground">Display Logic</h4>
                </div>
                {!readOnly && (
                    <button
                        onClick={(e) => {
                    ...
                    >
                        <span className="material-symbols-rounded text-base leading-none select-none">close</span>
                    </button>
                )}
            </div>
                ...
                <div className="flex flex-wrap gap-2 items-center">
                    ...
                    {confirmedConditions.map((c, i) => (
                        <React.Fragment key={c.id}>
                            {i > 0 && (
                                <span className="font-semibold text-foreground">{logic.operator}</span>
                            )}
                            <span
                                onClick={(e) => {
                                    e.stopPropagation();
                        ...
                        <React.Fragment key={s.id}>
                            ...
                            >
                                ...
                                {s.conditions.map((c, ci) => (
                                    <React.Fragment key={c.id}>
                                        {ci > 0 && (
                                            <span className="font-bold text-foreground"> {s.operator} </span>
                                        )}
                                        {fmt(c)}
                                    </React.Fragment>
                                ))}
...
function SkipLogicDisplayMock({
    ...
    const confirmedRules =
        logic.type === 'per_choice' ? logic.rules.filter((r) => r.isConfirmed) : [];
...
interface IncomingLogicDisplayMockProps {
    branchName: string;
    sourceQuestionId: string; // qid like "Q1"
...
function BranchingLogicDisplayMock({
    ...
    for (const branch of logic.branches) {
        if (branch.thenSkipToIsConfirmed) {
            for (const cond of branch.conditions) {
                if (cond.questionId === question.qid && cond.isConfirmed && cond.value) {
    ...
    const hasAnyConfirmed = logic.branches.some(
        (b) => b.thenSkipToIsConfirmed && b.conditions.some((c) => c.isConfirmed === true),
    );
        ...
        >
            ...
            <div className="flex items-center justify-between mb-2">
                <div className="flex items-center gap-2">
                    ...
                    <h4 className="text-sm font-medium text-foreground">Branching Logic</h4>
                </div>
            </div>
            ...
            <div className="space-y-2 text-sm">
                {logic.branches.map((branch) => {
                    const confirmed = branch.conditions.filter((c) => c.isConfirmed === true);
                    if (confirmed.length === 0 || !branch.thenSkipToIsConfirmed) return null;
                    return (
                        <div
                            key={branch.id}
                            onClick={(e) => {
                        ...
                        >
                            {branch.pathName && (
                                <div className="font-medium text-foreground mb-1">{branch.pathName}</div>
                            )}
                            <div className="text-foreground">
                                ...
                                {confirmed.length > 1 ? (
                                    <div className="ml-4 flex flex-col gap-1 my-1">
                                        {confirmed.map((c, ci) => (
                                            <div key={c.id}>
                                                {ci > 0 && (
                                                    <span className="font-semibold">{branch.operator} </span>
                                                )}
                                ...
                                <span className={confirmed.length > 1 ? 'mt-1 block' : ''}> THEN </span>
                                <span>
                                    skip to{' '}
                                    <span className="font-semibold">{fmtDest(branch.thenSkipTo, survey)}</span>
                                    .
                                </span>
                            </div>
                        </div>
                ...
                */}
                {showOtherwise && logic.otherwiseIsConfirmed && logic.otherwiseSkipTo && (
                    <div className="p-2 bg-card rounded-md border border-dashed border-border-ui text-sm text-muted-foreground">
                        ...
                        .
                        <span className="ml-2 text-xs text-muted-foreground/60 italic">(fallback)</span>
                    </div>
                )}
            </div>
...
interface SurveyFlowDisplayMockProps {
    logic: MockSkipLogicSimple;
    survey: MockSurvey;
    onClick?: () => void;
    sourceQuestion?: MockQuestion;
    allBranchingLogics?: Array<{ question: MockQuestion; logic: MockBranchingLogic }>;
}
...
const Q1 = SURVEY.blocks[0].questions[0]; // Q1 satisfaction (has choices)
const Q2 = SURVEY.blocks[0].questions[1]; // Q2 recommend (Yes/No)
...
} satisfies Meta<typeof DisplayLogicDisplayMock>;
...
export const DisplayLogic_MultipleRules: Story = {
    ...
    args: {
        logic: {
            operator: 'OR',
            conditions: [
                { id: 'cond-a', questionId: 'Q1', operator: 'equals', value: 'Very satisfied', isConfirmed: true },
                { id: 'cond-b', questionId: 'Q2', operator: 'equals', value: 'Yes', isConfirmed: true },
                { id: 'cond-c', questionId: 'Q5', operator: 'equals', value: '25–44', isConfirmed: true },
            ],
        },
    },
};
...
export const DisplayLogic_AllOperators: Story = {
    ...
    args: {
        logic: {
            operator: 'OR',
            conditions: [
                { id: 'op-eq',  questionId: 'Q1', operator: 'equals',        value: 'Very satisfied', isConfirmed: true },
                { id: 'op-neq', questionId: 'Q1', operator: 'not_equals',    value: 'Neutral',        isConfirmed: true },
                { id: 'op-con', questionId: 'Q3', operator: 'contains',      value: 'support',        isConfirmed: true },
                { id: 'op-gt',  questionId: 'Q3', operator: 'greater_than',  value: '5',              isConfirmed: true },
                { id: 'op-lt',  questionId: 'Q3', operator: 'less_than',     value: '3',              isConfirmed: true },
                { id: 'op-emp', questionId: 'Q4', operator: 'is_empty',                               isConfirmed: true },
                { id: 'op-nem', questionId: 'Q4', operator: 'is_not_empty',                           isConfirmed: true },
            ],
        },
    },
};
...
export const DisplayLogic_Nested: Story = {
    ...
    args: {
        logic: {
            operator: 'AND',
            conditions: [
                { id: 'flat-1', questionId: 'Q1', operator: 'equals', value: 'Very satisfied', isConfirmed: true },
            ],
            logicSets: [
                {
                    id: 'set-1',
                    operator: 'AND',
                    conditions: [
                        { id: 'nested-a', questionId: 'Q2', operator: 'equals', value: 'Yes', isConfirmed: true },
                        { id: 'nested-b', questionId: 'Q5', operator: 'equals', value: '25–44', isConfirmed: true },
                    ],
                    isConfirmed: true,
                },
            ],
    ...
    render: (args) => (
        <SkipLogicDisplayMock
            logic={{
                type: 'per_choice',
                rules: [
                    { choiceId: 'c-very-satisfied', skipTo: 'block:block-feedback',     isConfirmed: true },
                    { choiceId: 'c-satisfied',      skipTo: 'block:block-demographics', isConfirmed: true },
...
export const BranchingLogic_Default: Story = {
    ...
    render: (args) => (
        <BranchingLogicDisplayMock
            ...
            survey={args.survey}
            onClick={args.onClick as (id?: string) => void}
            onRemove={args.onRemove}
            issues={args.issues}
        />
    ),
...
export const SurveyFlow_Default: Story = {
    ...
    render: (args) => (
        <SurveyFlowDisplayMock
            logic={{ type: 'simple', isConfirmed: true, skipTo: 'block:block-demographics' }}
            survey={args.survey}
            sourceQuestion={Q1}
            onClick={args.onClick as () => void}
        />
            ...
            <div>
                ...
                <DisplayLogicDisplayMock
                    logic={{
                        operator: 'AND',
                        conditions: [
                            { id: 'ov-c1', questionId: 'Q2', operator: 'equals', value: 'Yes', isConfirmed: true },
                        ],
                    }}
                    survey={args.survey}
                        ...
                        branches: [
                            {
                                ...
                                operator: 'AND',
                                conditions: [
                                    { id: 'ov-bc', questionId: 'Q1', operator: 'equals', value: 'Very satisfied', isConfirmed: true },
                                ],
                                thenSkipTo: 'block:block-feedback',
                                thenSkipToIsConfirmed: true,
                            },
