...
const MOCK_QUESTIONS: MockQuestion[] = [
  {
    id: "Q1",
    label: "Q1 – What is your age?",
    choices: ["Under 18", "18–34", "35–54", "55+"],
  },
  { id: "Q2", label: "Q2 – Do you own a car?", choices: ["Yes", "No"] },
  {
    id: "Q3",
    label: "Q3 – Preferred contact method",
...
function ConditionRow({
  ...
  const selectedQuestion = MOCK_QUESTIONS.find(
    ...
    <div className="flex items-center gap-2">
      ...
      >
        <SelectTrigger
          ...
          aria-invalid={
            hasError && condition.questionId && !condition.operator
              ? "true"
              : undefined
          }
        >
      ...
      >
        <SelectTrigger
          ...
          aria-invalid={
            hasError && condition.operator && !condition.value
              ? "true"
              : undefined
          }
        >
...
function OperatorBadge({ label, isSelected, onClick }: OperatorBadgeProps) {
  ...
  const variant = isSelected ? "default" : hovered ? "secondary" : "outline";
...
function LogicSetDisplay({
  ...
  const [actionValue, setActionValue] = useState<"show" | "hide">(
    initialActionValue ?? "show",
  );
  ...
  const handleChange = (id: string, field: keyof Condition, value: string) => {
    setLogicSet((prev) => ({
      ...
      conditions: prev.conditions.map((c) => {
        if (c.id !== id) return c;
        if (field === "questionId")
          return { ...c, questionId: value, operator: "", value: "" };
  ...
  const handleApply = () => {
    ...
    logicSet.conditions.forEach((c) => {
      const requiresValue =
        c.operator && !["is_empty", "is_not_empty"].includes(c.operator);
  ...
  const freshSeed = (): LogicSetData => ({
    ...
    operator: "AND",
    conditions: [
      { id: `cond-${Date.now()}`, questionId: "", operator: "", value: "" },
    ],
    isConfirmed: false,
  });
          ...
          <div className="flex items-center gap-2 w-full">
            {initialActionValue !== undefined ? (
              <>
                <Select
                  value={actionValue}
                  onValueChange={(v) => setActionValue(v as "show" | "hide")}
                >
...
const confirmedSeed: LogicSetData = {
  ...
  operator: "AND",
  conditions: [
    { id: "cond-1", questionId: "Q2", operator: "equals", value: "Yes" },
  ],
  isConfirmed: true,
};
...
const sharedActionArgs = {
  actionValue: "show" as const,
  headerContent: Q3Header,
};
...
export const WithError: Story = {
  args: {
    initialData: {
      ...
      operator: "AND",
      conditions: [
        { id: "cond-err-1", questionId: "", operator: "", value: "" },
      ],
      isConfirmed: false,
    },
...
export const DisplayChoiceLogicSet: Story = {
  ...
  args: { initialData: unconfirmedSeed, actionValue: "show" },
  render: () => (
    <LogicSetDisplay
      initialData={unconfirmedSeed}
      actionValue="show"
...
export const AllStates: Story = {
  ...
  render: () => (
    <div className="flex flex-col gap-6 w-[480px]">
      ...
      <div>
        <p className="text-xs text-muted-foreground uppercase tracking-wider mb-2">
          Display question logic set
        </p>
        <LogicSetDisplay initialData={unconfirmedSeed} {...sharedActionArgs} />
      </div>
      <div>
        <p className="text-xs text-muted-foreground uppercase tracking-wider mb-2">
          Display choice logic set
        </p>
        <LogicSetDisplay
